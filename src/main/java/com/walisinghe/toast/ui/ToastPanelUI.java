package com.walisinghe.toast.ui;
...
import static com.walisinghe.toast.ToastClientProperties.*;
...
public class ToastPanelUI extends BasicPanelUI implements StyleableUI, PropertyChangeListener {
    ...
    protected void installDefaults(JPanel p) {
        ...
        closeButtonIcon = UIUtils.getIcon(prefix + ".closeIcon", UIUtils.createIcon("com/walisinghe/toast/svg/close.svg", closeIconColor, 0.75f));
    ...
    protected void closeButtonClicked(JComponent c) {
        ...
        if (callback instanceof Runnable) {
            ...
            // Check the actual type of the consumer
            // Ensure the consumer accepts the correct type
            if (isValidConsumer(component)) {
                // Safe to cast because isValidConsumer has validated it
                @SuppressWarnings("unchecked") Consumer<JComponent> specificConsumer = (Consumer<JComponent>) consumer;
                specificConsumer.accept(component);
            }
        }
    }

    private boolean isValidConsumer(JComponent component) {
        // Get the type of the Consumer's parameter using reflection
        Type consumerType = getConsumerType(component);

        // Check if the consumer's type matches JComponent
        return consumerType != null && consumerType.equals(JComponent.class);
    }

    private Type getConsumerType(JComponent component) {
        // Use reflection to get the generic type parameter of the Consumer
        Type[] genericInterfaces = component.getClass().getGenericInterfaces();
        for (Type type : genericInterfaces) {
            if (type instanceof ParameterizedType parameterizedType) {
                if (Consumer.class.equals(parameterizedType.getRawType())) {
